...
summary: Sie müssen ein DIMM im Controllermodul austauschen, wenn Ihr System eine zunehmende Anzahl korrigierbarer Fehlerkorrekturcodes registriert. Andernfalls kommt es zu einem Systemabsturz. 
---
= Ein DIMM-Modul im FAS500f-System ersetzen, um Speicherfehler zu beheben
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sie müssen ein DIMM im Controller austauschen, wenn Ihr Speichersystem Fehler wie übermäßige CECC-Fehler (korrigierbare Fehlerkorrekturcodes) aufgrund von Health Monitor-Warnmeldungen oder nicht korrigierbare ECC-Fehler aufweist. Diese Fehler werden typischerweise durch den Ausfall eines einzelnen DIMM verursacht, der das Booten von ONTAP im Speichersystem verhindert.
