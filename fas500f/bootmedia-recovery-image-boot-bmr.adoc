...
summary: Nach der Installation des neuen Bootmediums in Ihrem FAS500f-System können Sie den automatisierten Bootmedien-Wiederherstellungsprozess starten, um die Konfiguration vom Partnerknoten wiederherzustellen. 
---
= Das Wiederherstellungsimage auf einem FAS500f System mithilfe des automatisierten Boot-Wiederherstellungsprozesses starten
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Nach dem Einbau des neuen Bootmediums in Ihr FAS500f-System können Sie den automatischen Bootmedien-Wiederherstellungsprozess starten, um die Konfiguration vom Partnerknoten wiederherzustellen. Während des Wiederherstellungsprozesses prüft das System, ob die Verschlüsselung aktiviert ist und ermittelt den Typ der verwendeten Schlüsselverschlüsselung. Wenn die Schlüsselverschlüsselung aktiviert ist, führt Sie das System durch die entsprechenden Schritte zur Wiederherstellung.
...
.. Gehe zu <<reenable-auto-giveback,automatische Rückvergütung wieder aktivieren>> wenn es deaktiviert war.
...
`key manager is configured.`
 a| 
Die Verschlüsselung ist installiert. Gehe zu<<restore-key-manager,Wiederherstellung des Schlüsselmanagers>> .
...
. [[restore-key-manager]] Stellen Sie den Schlüsselmanager mithilfe der für Ihre Konfiguration geeigneten Vorgehensweise wieder her:
+
[role="tabbed-block"]
====
...
. [[reenable-auto-giveback]]Falls die automatische Rückgabe deaktiviert war, aktivieren Sie sie wieder:
+
...
.Wie es weiter geht
Nachdem das ONTAP Image wiederhergestellt wurde und der Knoten wieder betriebsbereit ist und Daten bereitstellt, link:bootmedia-complete-rma-bmr.html["Geben Sie das fehlerhafte Teil an NetApp zurück"].
