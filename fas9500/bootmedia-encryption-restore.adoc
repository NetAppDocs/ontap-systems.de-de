...
summary: Nachdem Sie die Umgebungsvariablen überprüft haben, müssen Schritte durchgeführt werden, die speziell für Systeme gelten, bei denen Onboard Key Manager OKM, NetApp Storage Encryption NSE oder NetApp Volume Encryption NVE aktiviert sind. 
---
= Die Verschlüsselung auf dem Startmedium Ihres FAS9500 Systems wird nach dem manuellen Boot-Wiederherstellungsprozess wiederhergestellt.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Nachdem das Bootmedium in Ihrem FAS9500-System ausgetauscht wurde, kann die Verschlüsselung wiederhergestellt werden, indem die Schritte für den jeweiligen Schlüsselmanagertyp, wie zum Beispiel den Onboard Key Manager (OKM), abgeschlossen werden.
