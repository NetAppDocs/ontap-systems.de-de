...
summary: Um das Gehäuse auszutauschen, müssen Sie die Komponenten aus dem Gehäuse für beeinträchtigte Geräte entfernen und sie in das Ersatzgehäuse einsetzen. 
---
= Das Chassis in einem FAS9500 System austauschen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Beim Austausch des Gehäuses müssen die sechs Lüftermodule an der Vorderseite des Gehäuses entfernt werden.
...
. Das Controller-Modul wird an einem sicheren Ort aufbewahrt, wobei festgehalten wird, aus welchem Chassis-Steckplatz es stammt, damit es im Ersatzgehäuse wieder in denselben Steckplatz eingesetzt werden kann.
...
Nachdem das Ersatzchassis in das Rack oder den Systemschrank eingebaut ist, müssen die De-Stage-Controller-Leistungsmodule wieder eingebaut werden.
