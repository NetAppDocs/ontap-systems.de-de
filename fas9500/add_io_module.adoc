...
---
= Ein E/A-Modul zu einem FAS9500 System hinzufügen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sie können Ihrem FAS9500 System ein E/A-Modul hinzufügen, indem entweder in einem vollständig bestückten System eine NIC oder ein Storage-Adapter durch einen neuen ersetzt wird oder indem eine neue NIC oder ein neuer Storage-Adapter in einen leeren Chassis-Slot eingesetzt wird.
...
NOTE: Stellen Sie sicher, dass Sie Blindabdeckungen in alle ungenutzten I/O-Steckplätze einsetzen, um mögliche thermische Probleme zu vermeiden.
...
. Wenn Sie Steckplatz 3 oder 7 für die Netzwerkverbindung verwenden, kann der Steckplatz mit dem  `storage port modify -node __<node name>__ -port __<port name>__ -mode network` Befehl für die Netzwerknutzung umgewandelt werden.
...
NIC zu NIC (gleiche Anzahl von Ports)
 a| 
Die LIFs migrieren automatisch, wenn das Controller-Modul heruntergefahren wird.
