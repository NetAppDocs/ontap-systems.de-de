...
---
= Das NVRAM Modul oder die NVRAM DIMMs in einem FAS9500 System ersetzen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Das NVRAM Modul in Ihrem FAS9500 System besteht aus dem NVRAM11 und den DIMMs. Ein ausgefallenes NVRAM Modul oder ein NVRAM DIMM kann ersetzt werden, damit Ihr System weiterhin normal funktioniert.

Um ein defektes NVRAM Modul zu ersetzen, muss es aus dem Gehäuse entfernt, die DIMMs in das Ersatzmodul umgesetzt und das Ersatz NVRAM Modul in das Gehäuse eingesetzt werden.

Um ein NVRAM DIMM auszutauschen, muss das NVRAM Modul aus dem Gehäuse entfernt, das defekte DIMM im Modul ersetzt und anschließend das NVRAM Modul wieder eingebaut werden.
...
Sie müssen die Änderung der System-ID bestätigen, wenn Sie den Ersatzcontroller starten, und anschließend überprüfen, ob die Änderung korrekt ist.
...
+
Die MetroCluster Konfiguration benötigt nach dem Austausch einige Minuten, um wieder den Normalzustand zu erreichen, zu welchem Zeitpunkt jeder Controller den konfigurierten Status anzeigt, mit aktiviertem DR Mirroring und dem Modus „Normal“. Die  `metrocluster node show -fields node-systemid` Befehlsausgabe zeigt die alte System-ID an, bis die MetroCluster Konfiguration wieder den Normalzustand erreicht.
