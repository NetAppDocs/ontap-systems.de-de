...
[role="lead"]
Ersetzen Sie ein E/A-Modul in Ihrem FAS50-Speichersystem, wenn das Modul ausfällt oder ein Upgrade benötigt, um eine höhere Leistung oder zusätzliche Funktionen zu unterstützen. Der Austauschprozess umfasst das Herunterfahren des Controllers, den Austausch des ausgefallenen I/O-Moduls, den Neustart des Controllers und die Rücksendung des ausgefallenen Moduls an NetApp.
