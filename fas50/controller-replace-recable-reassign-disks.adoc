...
summary: 'Geben Sie die Kontrolle über die Speicherressourcen an den Ersatzcontroller zurück, damit Ihr FAS50-System den Normalbetrieb wieder aufnehmen kann.  Das Rückgabeverfahren variiert je nach dem von Ihrem System verwendeten Verschlüsselungstyp: keine Verschlüsselung, Onboard Key Manager OKM-Verschlüsselung oder External Key Manager EKM-Verschlüsselung.' 
---
= Den Controller in Ihrem FAS50 System zurückgeben, damit der normale Betrieb fortgesetzt werden kann.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
