...
summary: Um den Controller-Austausch Ihres FAS50-Systems abzuschließen, stellen Sie zunächst die NetApp Storage Encryption-Konfiguration wieder her, falls erforderlich, und installieren Sie die erforderlichen Lizenzen auf dem neuen Controller. Überprüfen Sie anschließend, ob die logischen Schnittstellen an ihren Home-Ports melden, und führen Sie eine Cluster-Integritätsprüfung durch. Registrieren Sie abschließend die Seriennummer des neuen Controllers und senden Sie das defekte Teil an NetApp zurück. 
---
= Den Controller-Austausch für ein FAS50-System abschließen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Um den Controller-Austausch für Ihr FAS50-System abzuschließen, stellen Sie zuerst die Konfiguration der NetApp-Storage-Verschlüsselung (falls erforderlich) wieder her und installieren Sie die erforderlichen Lizenzen auf dem neuen Controller. Vergewissern Sie sich als nächstes, dass die logischen Schnittstellen (LIFs) ihren Home Ports berichten und eine Cluster-Zustandsprüfung durchführen. Registrieren Sie abschließend die Seriennummer des neuen Controllers und senden Sie das ausgefallene Teil an NetApp zurück.
