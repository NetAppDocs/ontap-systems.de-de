...
summary: Um den Controlleraustausch für Ihr FAS50-System abzuschließen, stellen Sie zunächst ggf. die NetApp Storage Encryption-Konfiguration wieder her und installieren Sie die erforderlichen Lizenzen auf dem neuen Controller.  Bestätigen Sie als Nächstes, dass die logischen Schnittstellen-LIFs an ihre Home-Ports berichten, und führen Sie eine Cluster-Integritätsprüfung durch.  Registrieren Sie abschließend die Seriennummer des neuen Controllers und senden Sie das defekte Teil anschließend an NetApp zurück. 
---
