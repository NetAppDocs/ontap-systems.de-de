...
summary: Tauschen Sie die Echtzeituhrbatterie, allgemein bekannt als Knopfzellenbatterie, in Ihrem AFF C30 oder AFF C60 Speichersystem aus, um sicherzustellen, dass Dienste und Anwendungen, die auf eine genaue Zeitsynchronisation angewiesen sind, weiterhin betriebsbereit bleiben. 
---
= Die Echtzeit Batterie der Echtzeituhr in einem AFF C30 oder AFF C60 System ersetzen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Bevor Sie beginnen
Alle anderen Komponenten des Speichersystems müssen einwandfrei funktionieren. Falls dies nicht der Fall ist, wenden Sie sich an  https://mysupport.netapp.com/site/global/dashboard["NetApp Support"] bevor Sie mit diesem Verfahren fortfahren.
...
+
Sie können sie durch Eingabe des  `system location-led _off_` Befehls ausschalten. Wenn Sie sich nicht sicher sind, ob die LEDs ein- oder ausgeschaltet sind, lässt sich ihr Status durch Eingabe des  `system location-led show` Befehls überprüfen.
...
NOTE: Nach dem Austausch der RTC-Batterie, dem Einsetzen des Controllers und dem Einschalten werden beim ersten BIOS-Reset möglicherweise die folgenden Fehlermeldungen angezeigt:
`RTC date/time error. Reset date/time to default`
`RTC power failure error` Diese Meldungen sind zu erwarten und der Vorgang kann fortgesetzt werden.
