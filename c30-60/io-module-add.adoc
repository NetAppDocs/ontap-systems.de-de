...
---
= Ein E/A-Modul kann zu Ihrem AFF C30 oder AFF C60 Speichersystem hinzugefügt werden, um die Netzwerkkonnektivität zu verbessern
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Erweitern Sie Ihr AFF C30- oder AFF C60-Speichersystem um ein I/O-Modul, um die Netzwerkanbindung zu verbessern und die Fähigkeit Ihres Systems zur Handhabung des Datenverkehrs zu erweitern. Mit diesem Verfahren können Sie ein I/O-Modul entweder im Rahmen eines geplanten Upgrades an einem betriebsbereiten Controller oder während der Wartung an einem beeinträchtigten Controller hinzufügen.
...
.Über diese Aufgabe
Diese Prozedur kann auf jedem der beiden Controller des HA-Paares durchgeführt werden, unabhängig davon, ob der Controller aktuell betriebsbereit oder beeinträchtigt ist. Da ein E/A-Modul nicht zu einem laufenden Controller hinzugefügt werden kann, muss der Zielcontroller vor der Installation des Moduls heruntergefahren werden. Der HA-Partner übernimmt während der Prozedur die Speicheroperationen, sodass die Daten für Clients weiterhin verfügbar bleiben.

* Wenn der Zielcontroller *betriebsbereit* ist (geplante Erweiterung), befolgen Sie die Abschaltschritte wie beschrieben und behandeln Sie den Zielcontroller in diesem Inhalt als den "beeinträchtigten" Controller.
* Falls der Zielcontroller *beeinträchtigt* ist, befolgen Sie die beschriebenen Schritte zum Herunterfahren.
...
Übernehmen Sie das Ziel-Controller-Modul und fahren Sie es herunter, damit dessen HA-Partner weiterhin Daten bereitstellt, während Sie das I/O-Modul hinzufügen. Die verlinkten Schritte beziehen sich auf den herunterzufahrenden Controller als „beeinträchtigten“ Controller; die gleichen Schritte gelten unabhängig davon, ob Sie einen betriebsbereiten Controller für eine geplante Erweiterung übernehmen oder einen bereits beeinträchtigten Controller herunterfahren.
