...
summary: Ergänzen Sie Ihr AFF C30- oder AFF C60-Speichersystem um ein E/A-Modul, um die Netzwerkanbindung zu verbessern und die Fähigkeit Ihres Systems zur Verarbeitung von Datenverkehr zu erweitern. 
---
= Ein E/A-Modul zu einem AFF C30 oder AFF C60 System hinzufügen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Fügen Sie Ihrem AFF C30 oder AFF C60 Speichersystem ein I/O-Modul hinzu, um die Netzwerkkonnektivität zu verbessern und die Fähigkeit Ihres Systems zur Verarbeitung von Datenverkehr zu erweitern. Mit diesem Verfahren kann ein I/O-Modul entweder als geplantes Upgrade an einem betriebsbereiten Controller oder während der Wartung an einem Controller mit einem beeinträchtigten I/O-Modul hinzugefügt werden.
...
.Über diese Aufgabe
Diese Prozedur kann auf jedem der beiden Controller im HA-Paar durchgeführt werden, unabhängig davon, ob das I/O-Modul des Controllers aktuell betriebsbereit oder beeinträchtigt ist. Da ein I/O-Modul nicht zu einem laufenden Controller hinzugefügt werden kann, muss der Zielcontroller vor der Installation des Moduls heruntergefahren werden. Der HA-Partner übernimmt während der Prozedur die Speicheroperationen, sodass die Daten für Clients weiterhin verfügbar bleiben.

* Wenn der Zielcontroller betriebsbereit ist (geplante Erweiterung), sind die Abschaltschritte wie beschrieben zu befolgen, wobei der Zielcontroller in diesem Verfahren so zu behandeln ist, als ob er ein beeinträchtigtes E/A-Modul hätte.
* Wenn der Zielcontroller ein defektes E/A-Modul hat, sind die Abschaltschritte wie beschrieben auszuführen.
...
== Schritt 1: Das Zielsteuerungsmodul herunterfahren

Den Zielcontroller übernehmen und herunterfahren, sodass sein HA-Partner weiterhin Daten bereitstellt, während das I/O-Modul hinzugefügt wird. Die verlinkten Schritte beziehen sich auf den Controller, der heruntergefahren wird und das beeinträchtigte I/O-Modul hostet. Diese Schritte gelten sowohl für die Übernahme eines Controllers im Rahmen einer geplanten Erweiterung als auch für das Herunterfahren eines Controllers, der bereits ein beeinträchtigtes I/O-Modul besitzt.
...
.Bevor Sie beginnen
* Überprüfung des  https://hwu.netapp.com["NetApp Hardware Universe"^] zur Sicherstellung, dass das neue E/A-Modul mit Ihrem Speichersystem und der von Ihnen verwendeten ONTAP-Version kompatibel ist.
