...
summary: Um den Controller-Austausch für Ihr AFF C30- oder AFF C60-Speichersystem abzuschließen, stellen Sie zunächst gegebenenfalls die NetApp Storage Encryption-Konfiguration wieder her und installieren Sie die erforderlichen Lizenzen auf dem neuen Controller.  Bestätigen Sie als Nächstes, dass die logischen Schnittstellen-LIFs an ihre Home-Ports berichten, und führen Sie eine Cluster-Integritätsprüfung durch.  Registrieren Sie abschließend die Seriennummer des neuen Controllers und senden Sie das defekte Teil an NetApp zurück. 
---
= Den Controller-Austausch für ein AFF C30 oder AFF C60 System abschließen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
