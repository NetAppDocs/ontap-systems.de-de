...
summary: Stellen Sie die Verschlüsselung auf dem Ersatz-Bootmedium in Ihrem AFF C30- oder AFF C60-Speichersystem wieder her, um den Datenschutz kontinuierlich zu gewährleisten. Der Austauschprozess umfasst die Überprüfung der Schlüsselverfügbarkeit, die erneute Anwendung der Verschlüsselungseinstellungen und die Bestätigung des sicheren Zugriffs auf Ihre Daten. 
---
= Die Verschlüsselung auf dem Bootmedium im AFF C30 oder AFF C60 Storage-System wird nach dem manuellen Boot-Wiederherstellungsprozess wiederhergestellt.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
