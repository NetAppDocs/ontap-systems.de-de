---
permalink: c800/install-setup.html 
sidebar: sidebar 
keywords: aff c800, initial configuration options 
summary: Installieren und richten Sie Ihr System ein. 
---
= Installation und Einrichtung eines AFF C800 Speichersystems
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Ein AFF C800 Speichersystem kann mit dem Inhaltsformat installiert und eingerichtet werden, das am besten zu Ihren Anforderungen passt: Kurzanleitung, Videoanleitung oder detaillierte Anleitung.

* link:../c800/install-quick-guide.html["Schnelle Schritte"]
+
Eine druckbare PDF-Datei mit Schritt-für-Schritt-Anweisungen mit Live-Links zu zusätzlichen Inhalten.

* link:../c800/install-videos.html["Videoschritte"]
+
Video Schritt-für-Schritt-Anleitungen.

* link:../c800/install-detailed-guide.html["Detaillierte Schritte"]
+
Schritt-für-Schritt-Anleitungen mit Live-Links zu weiteren Inhalten



Wenn sich das System in einer MetroCluster-IP-Konfiguration befindet, lesen Sie den https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/index.html["MetroCluster-IP-Konfiguration installieren"^] Anweisungen.
