...
summary: Das Bootmedium in Ihrem FAS2820 System speichert wichtige Firmware- und Konfigurationsdaten. Für den Austausch werden das defekte Controller-Modul entfernt und geöffnet, das Bootmedium lokalisiert und ersetzt, das Boot-Image auf ein USB-Laufwerk übertragen und anschließend der Controller gestartet. 
---
= Das Bootmedium in einem FAS2820 System mithilfe des manuellen Boot-Wiederherstellungsprozesses ersetzen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Das Bootmedium in Ihrem FAS2820 System speichert wichtige Firmware- und Konfigurationsdaten. Für den Austausch wird das defekte Controller-Modul entfernt und geöffnet, das Bootmedium lokalisiert und ersetzt, das Boot-Image auf ein USB-Laufwerk übertragen, das USB-Laufwerk in den Controller eingesetzt und anschließend der Controller gestartet.
...
Das Bootmedium befindet sich unter der Mezzanine-Karte im Controller-Modul. Die Anweisungen zum Austausch sind zu befolgen.
...
+
.. Verwenden Sie  https://kb.netapp.com/on-prem/ontap/DM/Encryption/Encryption-KBs/How_to_determine_if_the_running_ONTAP_version_supports_Data_At_Rest_Encryption["Wie sich feststellen lässt, ob Ihre ONTAP Version die NetApp Volume Encryption (NVE) unterstützt"^], um festzustellen, ob die Volume-Verschlüsselung derzeit unterstützt wird.
+
