...
summary: 'In Ihrem FAS2820-System werden die Netzteile, Festplatten und das/die Controllermodul(e) vom defekten Gehäuse in das Ersatzgehäuse umgesetzt, und das defekte Gehäuse wird im Geräteschrank oder Systemschrank durch das Ersatzgehäuse desselben Modells wie das defekte Gehäuse ersetzt.' 
---
= Das Chassis in einem FAS2820 System austauschen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
In Ihrem FAS2820-System werden die Netzteile, Festplatten und das/die Controllermodul(e) vom defekten Gehäuse in das Ersatzgehäuse umgesetzt, und das defekte Gehäuse wird im Geräteschrank oder Systemschrank durch das Ersatzgehäuse desselben Modells wie das defekte Gehäuse ersetzt.
...
. Das beschädigte Chassis mit Hilfe von zwei oder drei Personen von den Schienen im Systemschrank oder von den L-Winkeln im Geräteschrank herunterschieben und anschließend beiseitelegen.
. Wenn Sie nicht bereits geerdet sind, sollten Sie sich richtig Erden.
. Das Ersatzchassis wird mit Hilfe von zwei oder drei Personen in das Geräterack oder den Systemschrank eingesetzt, wobei das Chassis in einem Systemschrank auf die Rackschienen oder in einem Geräterack auf die L-Winkel geführt wird.
...
Das Controller-Modul und alle weiteren Komponenten werden in das Ersatzgehäuse eingesetzt und anschließend im Wartungsmodus gestartet.
...
+
.. Sobald jeder Controller mit dem Booten beginnt,  `Ctrl-C`drücken, um den Bootvorgang zu unterbrechen, wenn die Meldung  `Press Ctrl-C for Boot Menu` angezeigt wird.
+
