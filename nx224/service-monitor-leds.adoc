...
---
= Laufwerksregal-LEDs überwachen – NX224-Regale
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sie können den Zustand Ihres Shelfs überwachen, indem Sie die Position und den Status der LEDs an den Komponenten Ihres Laufwerks-Shelfs kennen.

* Die Standort-LEDs (blau) auf dem Bedieneranzeigefeld (ODP) eines Regals und auf beiden NVMe-Regalmodulen (NSMs) können aktiviert werden, um die physische Lokalisierung des zu wartenden Regals zu erleichtern: `storage shelf location-led modify -shelf-name _shelf_name_ -led-status on`
+
...
image::../media/drw_tp_nsmb_leds_ieops-2206.svg[NX224 NSM140 Modul-LEDs]
...
Die LEDs an einem Wechselstromnetzteil (PSU) zeigen an, ob das Netzteil normal funktioniert oder ob Hardwareprobleme vorliegen.

Die folgenden Abbildungen und Tabellen beschreiben die LED auf einem Netzteil.

image::../media/drw_generic_psu_ac_terra_cotta_ieops-2370.svg[Netzstrom-Aktivitäts-LED]
...
| Status | Leistung/Aktivität (grün) | Achtung (Rot) 
 ...
 a| 
Keine Wechselstromversorgung für das Gehäuse
 a| 
Aus
 a| 
Aus
 ...
 a| 
Keine Wechselstromversorgung für das Netzteil
 a| 
Aus
 a| 
Ein
 ...
 a| 
Wechselstrom eingeschaltet, aber Netzteil nicht im Gehäuse
 a| 
Blinken
 a| 
Aus
