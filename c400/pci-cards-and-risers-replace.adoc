...
---
= Austausch einer PCIe- oder Mezzanine-Karte im AFF C400 Storage-System
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Eine PCIe- oder Mezzanine-Karte im AFF C400-Speichersystem wird ersetzt, indem Kabel und SFP/QSFP-Module von den Karten getrennt, die defekte Karte ausgetauscht und die Verkabelung wiederhergestellt werden.
...
NOTE: Ziehen Sie die Rändelschrauben der Mezzanine-Karte vollständig fest. Sind sie locker, werden die Ports e0e bis e0h möglicherweise nicht erkannt, die LED blinkt dauerhaft, und in den Protokollen wird ein "unsupported cable"-Fehler angezeigt.
