...
summary: 'Sie müssen ein DIMM im Controllermodul austauschen, wenn Ihr System eine zunehmende Anzahl korrigierbarer Fehlerkorrekturcodes (ECC) registriert, oder wenn das Unterlassen zu einem Systemabsturz führt.' 
---
= Ein DIMM in Ihrem AFF C400 Speichersystem kann ausgetauscht werden, um Speicherfehler zu beheben.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ein DIMM in Ihrem AFF C400 Speichersystem sollte ersetzt werden, wenn das System auf übermäßig viele korrigierbare ECC-Fehler oder einen nicht korrigierbaren ECC-Fehler stößt, was typischerweise durch den Ausfall eines einzelnen DIMM verursacht wird.
