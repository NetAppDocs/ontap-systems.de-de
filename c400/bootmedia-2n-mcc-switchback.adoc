...
summary: 'Nachdem Sie in einer MetroCluster Konfiguration mit zwei Nodes den FRU-Austausch abgeschlossen haben, können Sie den MetroCluster SwitchBack-Vorgang durchführen. Damit wird die Konfiguration in ihren normalen Betriebszustand zurückversetzt, wobei die Synchronisations-Storage Virtual Machines (SVMs) auf dem ehemals beeinträchtigten Standort jetzt aktiv sind und Daten aus den lokalen Festplattenpools bereitstellen.' 
---
= Aggregate in einer Zwei-Knoten-MetroCluster-Konfiguration in Ihrem AFF C400 Storage-System zurückwechseln
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Nach Abschluss des Austauschs der Bootmedien in Ihrem AFF C400-Speichersystem in einer Zwei-Knoten MetroCluster Konfiguration ist die MetroCluster Switchback-Operation durchzuführen, um die Konfiguration in ihren normalen Betriebszustand zurückzusetzen.
