...
summary: Stellen Sie die Verschlüsselung auf den Ersatz-Startmedien in Ihrem AFF C80-System wieder her, um eine dauerhafte Datensicherheit zu gewährleisten. Der Austauschprozess umfasst die Überprüfung der Schlüsselverfügbarkeit, die Anwendung von Verschlüsselungseinstellungen und die Bestätigung des sicheren Zugriffs auf Ihre Daten. 
---
= Die Verschlüsselung auf dem Bootmedium Ihres AFF C80 Storage-Systems wird nach dem manuellen Boot-Wiederherstellungsprozess wiederhergestellt.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
