...
summary: Sie müssen ein DIMM im Controller austauschen, wenn in Ihrem Speichersystem Fehler auftreten, wie z. B. übermäßige korrigierbare Fehlerkorrekturcodes, die auf Warnungen des Health Monitors basieren, oder nicht korrigierbare ECC-Fehler, die typischerweise durch einen einzelnen DIMM-Ausfall verursacht werden und verhindern, dass das Speichersystem ONTAP startet. 
---
