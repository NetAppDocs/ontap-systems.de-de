...
summary: Der Controller in Ihrem FAS2600-System wird ersetzt, indem der defekte Controller entfernt, die FRU-Komponenten in das Ersatz-Controllermodul übertragen, dieses in das Gehäuse eingesetzt und das System im Wartungsmodus gestartet wird. 
---
= Den Controller in einem FAS2600 System ersetzen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Der Controller in Ihrem FAS2600-System wird ersetzt, indem der defekte Controller entfernt, die FRU-Komponenten in das Ersatz-Controllermodul übertragen, das Ersatz-Controllermodul in das Gehäuse eingesetzt und das System anschließend im Wartungsmodus gestartet wird.

link:https://youtu.be/2_uWRt_iJqk["Video: AFF FAS2600 Controller-Austausch"]
...
+
Der DIMM sitzt stramm im Steckplatz, sollte sich aber leicht einsetzen lassen. Falls dies nicht der Fall ist, den DIMM erneut am Steckplatz ausrichten und wieder einsetzen.
...
Um das Caching-Modul, das auf dem Controller-Etikett als M.2 PCIe Karte bezeichnet wird, vom alten Controller Modul auf das Ersatz Controller Modul zu übertragen, ist folgende Schrittfolge einzuhalten.

Das neue Controller-Modul muss bereitstehen, damit das Caching-Modul direkt vom alten Controller-Modul in den entsprechenden Steckplatz des neuen Moduls eingesetzt werden kann. Alle anderen Komponenten im Storage-System müssen ordnungsgemäß funktionieren. Ist dies nicht der Fall, sollte der technische Support kontaktiert werden.
...
Bei HA-Paaren mit zwei Controller-Modulen im selben Chassis ist die Reihenfolge, in der die Controller-Module installiert werden, besonders wichtig, da ein Controller-Modul versucht, neu zu starten, sobald es vollständig im Chassis sitzt.
...
NOTE: Die restlichen Kabel werden im weiteren Verlauf dieses Verfahrens an das Controller-Modul angeschlossen.
