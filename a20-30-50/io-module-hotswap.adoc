...
summary: Sie können ein Ethernet-E/A-Modul in Ihrem AFF A20, AFF A30 oder AFF A50 Speichersystem per Hot-Swap austauschen, wenn ein Modul ausfällt und Ihr Speichersystem alle ONTAP Versionserfordernisse erfüllt. 
---
= Hot-Swap eines I/O-Moduls in einem AFF A20, AFF A30 oder AFF A50 System
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sie können ein Ethernet-E/A-Modul in Ihrem AFF A20, AFF A30 oder AFF A50 Speichersystem per Hot-Swap austauschen, wenn ein Modul ausfällt und Ihr Speichersystem alle ONTAP Versionserfordernisse erfüllt.

Für den Hot-Swap eines E/A-Moduls ist sicherzustellen, dass das Speichersystem die ONTAP-Versionsanforderungen erfüllt, das Speichersystem und das E/A-Modul vorbereitet werden, das ausgefallene Modul im Hot-Swap-Verfahren ersetzt wird, das Ersatzmodul online geschaltet wird, der normale Betrieb des Speichersystems wiederhergestellt wird und das ausgefallene Modul an NetApp zurückgegeben wird.
...
+
** Der _beeinträchtigte Controller_ ist der Controller, an dem Sie das I/O-Modul im laufenden Betrieb austauschen.
...
+
Sie können sie durch Eingabe des  `system location-led _off_` Befehls ausschalten. Wenn Sie sich nicht sicher sind, ob die LEDs ein- oder ausgeschaltet sind, kann ihr Status durch Eingabe des  `system location-led show` Befehls überprüft werden.
