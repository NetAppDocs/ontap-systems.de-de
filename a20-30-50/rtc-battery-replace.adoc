...
summary: Tauschen Sie die Echtzeituhrbatterie, allgemein bekannt als Knopfzellenbatterie, in Ihrem AFF A20, AFF A30 oder AFF A50 Speichersystem aus, um sicherzustellen, dass Dienste und Anwendungen, die auf eine genaue Zeitsynchronisation angewiesen sind, weiterhin betriebsbereit bleiben. 
---
= Die Echtzeit-Uhr-Batterie in Ihrem AFF A20, AFF A30 oder AFF A50 System kann ersetzt werden.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ersetzen Sie die Echtzeituhrbatterie (RTC), allgemein als Knopfzellenbatterie bekannt, in Ihrem AFF A20-, AFF A30- oder AFF A50-Speichersystem, um sicherzustellen, dass Dienste und Anwendungen, die auf eine genaue Zeitsynchronisierung angewiesen sind, weiterhin betriebsbereit bleiben.

.Bevor Sie beginnen
Alle anderen Komponenten des Speichersystems müssen einwandfrei funktionieren. Wenn dies nicht der Fall ist, wenden Sie sich an  https://mysupport.netapp.com/site/global/dashboard["NetApp Support"] bevor Sie mit diesem Verfahren fortfahren.
...
|===
. Die Rändelschraube wird gegen den Uhrzeigersinn gedreht, bis sie gelockert ist, anschließend wird die Abdeckung des Controllers geöffnet.
...
NOTE: Nachdem Sie die RTC-Batterie ausgetauscht, den Controller eingesetzt und das System zum ersten BIOS-Reset eingeschaltet haben, werden die folgenden Fehlermeldungen angezeigt:
`RTC date/time error. Reset date/time to default`
`RTC power failure error` Diese Meldungen sind zu erwarten und Sie können mit diesem Vorgang fortfahren.
...
NOTE: Wenn das Speichersystem im Bootmenü anhält, die Option für  `Reboot node` auswählen und bei Aufforderung mit _y_ antworten, anschließend durch Drücken von _Strg+C_ zum LOADER booten.
...
. An der LOADER-Eingabeaufforderung  `bye`eingeben, um die E/A-Module und andere Komponenten neu zu initialisieren und den Controller neu zu starten.
