...
[role="lead"]
Stellen Sie die Konfiguration der NetApp-Storage-Verschlüsselung wieder her (falls erforderlich), installieren Sie die Lizenzen für den Ersatz-Controller, überprüfen Sie, ob die logischen Schnittstellen (LIFs) an ihren Home Port berichten, führen Sie eine Cluster-Integritätsprüfung durch, registrieren Sie die Seriennummer des Ersatz-Controllers und senden Sie das fehlerhafte Teil an NetApp zurück, um den letzten Schritt beim Austausch des AFF A20, AFF A30 oder AFF A50 Controllers abzuschließen.
