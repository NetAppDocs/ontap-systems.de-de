...
summary: Ein Laufwerk in Ihrem ASA C800 System wird im laufenden Betrieb ausgetauscht, wenn die Plattform eine Warnmeldung an die Systemkonsole ausgibt, die auf einen Laufwerksausfall hinweist. Zusätzlich leuchten sowohl die Fehler-LED auf dem Bedienerdisplay als auch die Fehler-LED am ausgefallenen Laufwerk. 
---
= Laufwerk im laufenden Betrieb in einem ASA C800 System austauschen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
In Ihrem ASA C800 System kann ein Laufwerk im laufenden Betrieb ausgetauscht werden, wenn ein Laufwerk ausfällt oder ein Upgrade erforderlich ist. Der Austauschprozess umfasst die Identifizierung des defekten Laufwerks, dessen sichere Entfernung und die Installation eines neuen Laufwerks, um den fortlaufenden Datenzugriff und die Systemleistung sicherzustellen.
