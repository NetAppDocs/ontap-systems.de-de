...
summary: Eine Shelf-ID in einem System kann geändert werden, wenn ONTAP noch nicht läuft, wenn ein Shelf im laufenden Betrieb hinzugefügt wird, bevor es mit dem System verkabelt ist, oder wenn ONTAP bereits läuft und alle Laufwerke im Shelf unzugeordnet, als Ersatzteile vorhanden oder Teil offline genommener Aggregate sind. 
---
= DS212C-, DS224C- oder DS460C-Regal-ID ändern
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sie können die Shelf ID in einem DS212C, DS224C oder DS460C Shelf ändern, wenn ONTAP noch nicht ausgeführt wird oder wenn ein Shelf im laufenden Betrieb hinzugefügt wird, bevor es mit dem System verbunden wird. Die Shelf ID kann auch geändert werden, wenn ONTAP betriebsbereit ist (Controller-Module zur Datenbereitstellung verfügbar sind) und alle Laufwerke im Shelf nicht zugeordnet, Ersatzteile oder Teil eines oder mehrerer offline genommener Aggregate sind.
...
. Wiederholen Sie die vorhergehenden Schritte für jedes weitere Festplattenfach.
