...
summary: Ein Disk Shelf kann im laufenden Betrieb entfernt oder ersetzt werden, wenn es aus einem eingeschalteten System entfernt oder ersetzt werden muss und I/O-Vorgänge stattfinden. 
---
= Ein DS212C, DS224C oder DS460C Shelf im laufenden Betrieb entfernen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sie können ein Disk Shelf im laufenden Betrieb entfernen, wenn es aus einem eingeschalteten System, bei dem E/A-Vorgänge stattfinden, entfernt oder ersetzt werden muss. Sie können ein oder mehrere Disk Shelves an beliebiger Stelle innerhalb eines Stapels von Disk Shelves entfernen oder einen Stapel von Disk Shelves ausbauen.
...
.. Falls erforderlich, fahren Sie mit der Reaktivierung der automatischen Zuweisung fort (mehrere Schritte weiter unten).
...
+
** Die Module IOM12, IOM12B oder IOM12C sind wie in einem DS224C oder DS212C Disk Shelf nebeneinander angeordnet; bei einem DS460C sind die Module IOM12, IOM12B oder IOM12C übereinander angeordnet.
...
Wenn ein DS460C Gehäuse in einen anderen Teil des Rechenzentrums verlegt oder das Gehäuse an einen anderen Standort transportiert wird, müssen die Laufwerke aus den Laufwerksschubladen entfernt werden, um mögliche Schäden an den Laufwerksschubladen und Laufwerken zu vermeiden.
