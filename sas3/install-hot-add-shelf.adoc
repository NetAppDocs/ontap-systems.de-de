...
summary: Sie können ein oder mehrere Disk Shelves im laufenden Betrieb zu einem bestehenden Stapel von Disk Shelves hinzufügen oder einen Stapel von einem oder mehreren Disk Shelves direkt an eine SAS HBA oder einen Onboard SAS Port des Controllers anschließen. 
---
= DS212C, DS224C oder DS460C Shelf im laufenden Betrieb hinzufügen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sie können ein oder mehrere Disk Shelves im laufenden Betrieb zu einem bestehenden Stapel von Disk Shelves hinzufügen oder einen Stapel von einem oder mehreren Disk Shelves direkt an eine SAS HBA oder einen Onboard SAS Port des Controllers anschließen.
...
* Stellen Sie sicher, dass Ihr System bestimmte Anforderungen erfüllt, bevor im laufenden Betrieb Festplattengehäuse mit IOM12, IOM12B oder IOM12C Modulen hinzugefügt werden:
+
...
+
** Wenn Sie ein Disk Shelf mit IOM12, IOM12B oder IOM12C Modulen im laufenden Betrieb zu einem bestehenden Stack (von Disk Shelfs mit IOM12, IOM12B oder IOM12C Modulen) hinzufügen, kann das Disk Shelf an einem der beiden Enden, dem logischen ersten oder letzten Disk Shelf, des Stacks im laufenden Betrieb hinzugefügt werden.
+
...
** Disk Shelfs mit IOM12, IOM12B oder IOM12C Modulen müssen in einem eigenen eindeutigen Stack untergebracht sein.
...
CAUTION: *Möglicher Verlust des Datenzugriffs:* Wenn das Shelf in Zukunft in einen anderen Teil des Rechenzentrums verlegt oder an einen anderen Standort transportiert wird, müssen die Laufwerke aus den Laufwerksschubladen entfernt werden, um mögliche Schäden an den Laufwerksschubladen und Laufwerken zu vermeiden.
...
+
Sie deaktivieren die automatische Zuweisung der Datenträgerbesitzrechte, bevor Sie die im laufenden Betrieb hinzugefügten Datenträgergehäuse verkabeln, und aktivieren sie dann später in diesem Verfahren wieder, nachdem Sie die im laufenden Betrieb hinzugefügten Datenträgergehäuse verkabelt haben.
...
. Wenn Sie einen Stapel Disk-Shelches direkt an einen Controller anschließen, führen Sie die folgenden Teilschritte aus; andernfalls fahren Sie mit dem nächsten Schritt fort.
+
...
Wenn Sie DS460C Shelves in Zukunft in einen anderen Teil des Rechenzentrums verlegen oder an einen anderen Standort transportieren, müssen die Laufwerke aus den Laufwerksschubladen entfernt werden, um mögliche Schäden an den Laufwerksschubladen und Laufwerken zu vermeiden.
