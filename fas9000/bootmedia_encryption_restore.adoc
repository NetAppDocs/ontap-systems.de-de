...
summary: 'Sobald Umgebungsvariablen geprüft werden, müssen Sie spezifische Schritte für Systeme mit aktiviertem Onboard Key Manager (OKM), NetApp Storage Encryption (NSE) oder NetApp Volume Encryption (NVE) durchführen.' 
---
= Stellen Sie OKM, NSE und NVE nach Bedarf wieder her – AFF A700 und FAS9000
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Wenn NSE oder NVE aktiviert sind und der Onboard Key Manager aktiviert ist, wechseln Sie zu  1: Restore NVE or NSE when Onboard Key Manager is enabled.
* Wenn NSE oder NVE für ONATP 9.5 aktiviert sind, finden Sie unter  2: Restore NSE/NVE on systems running ONTAP 9.5 and earlier.
* Wenn NSE oder NVE für ONTAP 9.6 aktiviert sind, finden Sie unter  3: Restore NSE/NVE on systems running ONTAP 9.6 and later.
