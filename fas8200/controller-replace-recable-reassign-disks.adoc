...
summary: Nach dem Austausch des Controllers das System wieder anschließen und bestätigen, dass die Festplattenzuordnung neu zugewiesen wurde, damit Ihr FAS8200 System den normalen Betrieb wieder aufnehmen kann. 
---
= Der Controller in Ihrem FAS8200 System wird zurückgegeben, damit der normale Betrieb fortgesetzt werden kann.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Nach dem Austausch des Controllers das System wieder anschließen und bestätigen, dass die Festplattenzuordnung neu zugewiesen wurde, damit Ihr FAS8200 System den normalen Betrieb wieder aufnehmen kann.
...
... Warten Sie `savecore` Befehl zum Abschließen, bevor Sie das Giveback ausgeben.
+
Sie können den folgenden Befehl eingeben, um den Fortschritt von zu überwachen `savecore` Befehl: `system node run -node _local-node-name_ partner savecore -s`
