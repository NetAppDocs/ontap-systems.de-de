...
[role="lead"]
Tauschen Sie das Gehäuse des ASA A70 oder ASA A90-Systems aus, wenn es aufgrund eines Hardwarefehlers erforderlich ist. Beim Austausch werden die Controller und Netzteile (PSUs) entfernt, die Laufwerke entfernt, das Ersatzgehäuse installiert und die Gehäusekomponenten wieder eingebaut.
...
Nachdem Sie das beeinträchtigte ASA A70 oder ASA A90 Chassis ersetzt und die Komponenten wieder eingebaut haben, müssen Sie link:chassis-replace-complete-system-restore-rma.html["Schließen Sie den Austausch des Gehäuses ab"].
