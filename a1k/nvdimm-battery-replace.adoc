...
summary: Tauschen Sie die NV-Batterie in Ihrem AFF A1K-System aus, wenn die Batterie anfängt, den Akku zu laden oder ausfällt, da sie für die Aufbewahrung wichtiger Systemdaten bei Stromausfällen verantwortlich ist. Beim Austausch wird der Controller heruntergefahren, das Controller-Modul entfernt, die NV-Batterie ersetzt, das Controller-Modul wieder eingebaut und das fehlerhafte Teil wieder an den NetApp zurückgegeben. 
---
= Die NV-Batterie in einem AFF A1K System ersetzen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
