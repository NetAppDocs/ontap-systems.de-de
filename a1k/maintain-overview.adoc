...
[role="lead"]
Möglicherweise müssen Sie Wartungsverfahren für Ihre Hardware durchführen. In diesem Abschnitt finden Sie spezifische Verfahren zur Wartung Ihrer AFF A1K-Systemkomponenten.

Bei den Verfahren in diesem Abschnitt wird vorausgesetzt, dass das AFF A1K System bereits als Storage Node in der ONTAP Umgebung implementiert wurde.
