...
summary: Tauschen Sie das Netzteil Ihres AFX 1K-Speichersystems aus, wenn es defekt ist oder einen Fehler aufweist, um sicherzustellen, dass Ihr System weiterhin die erforderliche Stromversorgung für einen stabilen Betrieb erhält. Der Austauschvorgang umfasst das Trennen des Ziel-Netzteils, das Abziehen des Netzkabels, das Entfernen des defekten Netzteils und den Einbau des Ersatz-Netzteils sowie das erneute Anschließen an die Stromquelle. 
---
= Netzteil im laufenden Betrieb in Ihrem AFX 1K Speichersystem austauschen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ersetzen Sie ein AC-Netzteil (PSU) in Ihrem AFX 1K-Speichersystem, wenn es ausfällt oder defekt ist, und stellen Sie so sicher, dass Ihr System weiterhin die erforderliche Leistung für einen stabilen Betrieb erhält.  Der Austauschvorgang umfasst das Trennen des Zielnetzteils, das Abziehen des Netzkabels, das Entfernen des fehlerhaften Netzteils und das Installieren des Ersatznetzteils sowie das anschließende erneute Anschließen an die Stromquelle.
