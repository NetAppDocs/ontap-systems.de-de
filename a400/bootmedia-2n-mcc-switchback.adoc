...
summary: Nach Abschluss des FRU-Austauschs in einer Zwei-Knoten-MetroCluster-Konfiguration führen Sie die MetroCluster-Switchback-Operation durch, um die Konfiguration in ihren normalen Betriebszustand zurückzuversetzen, wobei die Sync-Source Storage Virtual Machines am ehemals beeinträchtigten Standort nun aktiv sind und Daten aus den lokalen Festplattenpools bereitstellen. 
---
= Aggregate in einer Zwei-Knoten-MetroCluster Konfiguration in Ihrem AFF A400 System zurückwechseln
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
