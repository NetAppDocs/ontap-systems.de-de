...
NOTE: Wenn die Port-Etiketten auf der Karte nicht sichtbar sind, überprüfen Sie die Ausrichtung der Karteninstallation (der PCIe-Anschluss befindet sich auf der linken Seite des Kartensteckplatzes in den Karten A400 und FAS8300/8700), und suchen Sie dann nach der Karte nach der Teilenummer im https://["NetApp Hardware Universe"^] Für eine Grafik der Blende, in der die Port-Bezeichnungen angezeigt werden. Die Teilenummer der Karte kann über gefunden werden `sysconfig -a` Befehl oder auf der Packliste des Systems.
