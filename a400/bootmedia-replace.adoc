...
summary: Sie müssen das Bootmedium im Controllermodul lokalisieren und dann die Anweisungen zum Austausch befolgen. 
---
= Das Bootmedium im AFF A400-System kann mithilfe des manuellen Boot-Wiederherstellungsprozesses ersetzt werden.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Das Bootmedium in Ihrem AFF A400 System speichert wichtige Firmware- und Konfigurationsdaten. Der Austauschvorgang umfasst die Entfernung des Controllermoduls, die Entfernung des defekten Bootmediums, das Einsetzen des Ersatz-Bootmediums und anschließend die Übertragung des ONTAP Image auf das Ersatz-Bootmedium mithilfe eines USB-Flash-Laufwerks.
