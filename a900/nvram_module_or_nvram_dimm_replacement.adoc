...
NOTE: Diese Vorgehensweise gilt nur für Systeme, auf denen ONTAP in einem HA-Paar ausgeführt wird.


CAUTION: Eine Neuzuordnung von Festplatten ist nur erforderlich, wenn das NVRAM-Modul ersetzt wird.
...
. Wenn die Speicherverschlüsselung aktiviert ist, müssen Sie ************
...
[role="lead"]
Gehen Sie wie folgt vor, wenn die Speicherverschlüsselung aktiviert ist.
...
. Geben Sie an der Eingabeaufforderung die integrierte Schlüsselverwaltung-Passphrase für das Cluster ein: Beispiel hier brauchen******
. Geben Sie die Backup-Schlüsseldaten ein
+

NOTE: Nach Abschluss der Informationen zur Passphrase kehrt der Knoten zum Boot-Menü zurück.
...
. Geben Sie die Informationen zum Management-Zertifikat an den Eingabeaufforderungen: Beispiel brauchen hier*****
+

NOTE: Nachdem die Informationen zum Managementzertifikat abgeschlossen sind, kehrt der Node zum Boot-Menü zurück.
...
--
====
. MSID für die Festplatten zurücksetzen:
+
.. Wenn Sie integrierte Speicherverschlüsselung verwenden, gehen Sie wie in beschrieben vor https://docs.netapp.com/us-en/ontap/encryption-at-rest/assign-authentication-keys-seds-onboard-task.html["Zuweisen eines Datenauthentifizierungsschlüssels zu einem FIPS-Laufwerk oder einer SED (Onboard Key Management)"^]
.. Ich Sie verwenden externen keymanager, folgen Sie dem Verfahren in https://docs.netapp.com/us-en/ontap/encryption-at-rest/assign-authentication-keys-seds-external-task.html["Zuweisen eines Datenauthentifizierungsschlüssels zu einem FIPS-Laufwerk oder einer SED (External Key Management)"^]
