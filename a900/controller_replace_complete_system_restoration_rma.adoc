...
summary: Um Ihr AFF A900-System wieder voll funktionsfähig zu machen, verkabeln Sie den Speicher neu, stellen Sie die NetApp Storage Encryption-Konfiguration gegebenenfalls wieder her und installieren Sie Lizenzen für den neuen Controller. 
---
= Den Controller-Austausch für ein AFF A900 System abschließen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Zum Abschluss des Controller-Austauschs für Ihr AFF A900 System wird die NetApp Storage Encryption Konfiguration (falls erforderlich) wiederhergestellt, bestätigt, dass die logischen Schnittstellen (LIFs) an ihren Home-Ports melden, eine Cluster-Integritätsprüfung durchgeführt und das defekte Teil an NetApp zurückgesendet.
