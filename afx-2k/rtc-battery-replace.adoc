...
summary: Tauschen Sie die Echtzeituhr-Batterie in Ihrem AFX 2K-Speichersystem aus, um sicherzustellen, dass Dienste und Anwendungen, die auf eine genaue Zeitsynchronisation angewiesen sind, weiterhin betriebsbereit bleiben. 
---
= Die Echtzeit-Uhr-Batterie im AFX 2K Storage-System austauschen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Die Echtzeituhrbatterie (RTC-Batterie), auch als Knopfzellenbatterie bekannt, im AFX 2K Storage-System sollte ersetzt werden, damit Dienste und Anwendungen, die auf eine genaue Zeitsynchronisation angewiesen sind, weiterhin funktionsfähig bleiben.
