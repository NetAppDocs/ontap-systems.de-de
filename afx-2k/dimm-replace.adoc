...
summary: Ein DIMM in Ihrem AFX 2K Speichersystem sollte ersetzt werden, wenn übermäßige korrigierbare oder nicht korrigierbare Speicherfehler erkannt werden. Solche Fehler können verhindern, dass das Speichersystem ONTAP startet. Der Austauschprozess umfasst das Herunterfahren des beeinträchtigten Controllers, dessen Ausbau, den Austausch des DIMM, den Wiedereinbau des Controllers und anschließend die Rücksendung des defekten Teils an NetApp. 
---
= Ein DIMM in Ihrem AFX 2K-Speichersystem ersetzen, um Speicherfehler zu beheben
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
