...
summary: Die NV-Batterie in Ihrem AFX 2K Storage-System sollte ersetzt werden, wenn die Batterie an Ladung verliert oder ausfällt, da sie für die Sicherung kritischer Systemdaten bei Stromausfällen verantwortlich ist. Der Austauschprozess umfasst das Herunterfahren des beeinträchtigten Controllers, das Entfernen des Controller-Moduls, das Ersetzen der NV-Batterie, das Wiedereinsetzen des Controller-Moduls und die Rücksendung des defekten Teils an NetApp. 
---
= Die NV-Batterie im AFX 2K-Speichersystem ersetzen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
