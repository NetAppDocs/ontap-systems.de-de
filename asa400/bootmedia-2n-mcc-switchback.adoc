...
summary: 'Nachdem Sie den FRU-Austausch in einer Zwei-Knoten-MetroCluster-Konfiguration abgeschlossen haben, können Sie die MetroCluster-Rückschaltung durchführen. Dadurch wird die Konfiguration in ihren normalen Betriebszustand zurückversetzt, wobei die Sync-Source Storage Virtual Machines (SVMs) am zuvor beeinträchtigten Standort nun aktiv sind und Daten aus den lokalen Festplattenpools bereitstellen.' 
---
= Aggregate in einer Zwei-Knoten-MetroCluster Konfiguration in Ihrem ASA A400 System zurückwechseln
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
