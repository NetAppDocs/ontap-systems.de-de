...
[role="lead"]
Um den Controller-Austausch für Ihr AFF A70- und AFF A90-System abzuschließen, stellen Sie zuerst die NetApp-Speicherverschlüsselungskonfiguration (falls erforderlich) wieder her und installieren Sie die erforderlichen Lizenzen auf dem neuen Controller. Vergewissern Sie sich als nächstes, dass die logischen Schnittstellen (LIFs) ihren Home Ports berichten und eine Cluster-Zustandsprüfung durchführen. Registrieren Sie abschließend die Seriennummer des neuen Controllers und senden Sie das ausgefallene Teil an NetApp zurück.
