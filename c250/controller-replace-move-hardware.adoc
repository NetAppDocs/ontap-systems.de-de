...
---
= Den Controller in Ihrem AFF C250-Speichersystem ersetzen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Tauschen Sie den Controller in Ihrem AFF C250-System aus, wenn ein Hardwaredefekt dies erfordert. Der Austauschvorgang umfasst das Entfernen des defekten Controllers, das Umsetzen der Komponenten auf den Ersatzcontroller, den Einbau des Ersatzcontrollers und dessen Neustart.
...
Nachdem Sie den defekten AFF C250 Controller ausgetauscht haben, müssen Sie link:controller-replace-system-config-restore-and-verify.html["Systemkonfiguration wiederherstellen und verifizieren"].
