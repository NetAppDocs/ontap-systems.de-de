---
permalink: c250/install-videos.html 
sidebar: sidebar 
keywords: aff c250, hardware installation and cabling video 
summary: Das folgende Video zeigt, wie Sie Ihr neues System installieren und verkabeln. 
---
= Installation eines AFF C250 Speichersystems mithilfe von Videoanleitungen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Eine Videodemonstration zeigt, wie ein AFF C250 Storage System installiert und verkabelt wird.

.Animation - Installation und Einrichtung eines AFF C250
video::c6906786-b302-4c14-b39b-afc50062aac5[panopto]
Wenn Sie über eine MetroCluster-Konfiguration verfügen, verwenden Sie den MetroCluster-Installationsinhalt.

https://docs.netapp.com/us-en/ontap-metrocluster/index.html["MetroCluster-Dokumentation"^]
