...
summary: Tauschen Sie ein DIMM in Ihrem AFF A250 Controller aus, wenn das System aufgrund von Health Monitor-Warnungen übermäßig viele CECC-Fehler oder nicht korrigierbare ECC-Fehler feststellt, die das Speichersystem am Starten von ONTAP hindern. 
---
= Ein DIMM in einem AFF A250 System ersetzen, um Speicherfehler zu beheben
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ein DIMM in Ihrem AFF A250 System sollte ersetzt werden, wenn das Speichersystem eine übermäßige Anzahl korrigierbarer (CECC) oder nicht korrigierbarer Speicherfehler feststellt. Solche Fehler können verhindern, dass das Speichersystem ONTAP startet.
