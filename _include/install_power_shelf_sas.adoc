...
* Sie müssen ein Shelf aus- und wieder einschalten (schalten Sie den Netzschalter an der Rückseite des SAS-Shelf aus und wieder ein, warten Sie die entsprechende Zeit und schalten Sie dann das Gerät wieder ein), damit die Shelf-ID wirksam wird.
