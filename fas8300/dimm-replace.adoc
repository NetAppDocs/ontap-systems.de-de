...
summary: Sie müssen ein DIMM im Controller austauschen, wenn Ihr Speichersystem auf Fehler stößt, wie z. B. übermäßig viele korrigierbare Fehlerkorrekturcodes oder nicht korrigierbare Fehler, die typischerweise durch einen einzelnen DIMM-Ausfall verursacht werden, der das Speichersystem am Booten von ONTAP hindert. 
---
= Ein DIMM in Ihrem FAS8300 oder FAS8700 System wird ersetzt, um Speicherfehler zu beheben.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sie müssen ein DIMM im Controller austauschen, wenn Ihr Speichersystem Fehler wie übermäßige CECC-Fehler (Correctable Error Correction Code) aufgrund von Health Monitor-Warnmeldungen oder nicht korrigierbare ECC-Fehler aufweist. Diese Fehler werden typischerweise durch den Ausfall eines einzelnen DIMM verursacht, der das Booten von ONTAP im Speichersystem verhindert.

Alle anderen Systemkomponenten müssen einwandfrei funktionieren. Wenn eine Komponente nicht ordnungsgemäß funktioniert, ist der technische Support zu kontaktieren.
...
Anzahl und Position der DIMMs in Ihrem System hängen vom jeweiligen Systemmodell ab. Weitere Informationen bietet der FRU-Plan am Luftkanal.
...
+
.. Das Controller-Modul mithilfe der Verriegelungsriegel fest in das Gehäuse schieben, bis sich die Verriegelungsriegel anzuheben beginnen.
+
