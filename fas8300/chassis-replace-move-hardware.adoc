...
---
= Das Chassis in Ihrem FAS8300 oder FAS8700 System ersetzen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Wenn Sie nicht bereits geerdet sind, sollten Sie sich richtig Erden.
. Zu zweit wird das Ersatzgehäuse in den Geräteschrank oder Systemschrank eingesetzt, indem das Gehäuse auf die Schienen im Systemschrank oder Geräteschrank geführt wird.
...
Bei HA-Paaren mit zwei Controller-Modulen im selben Gehäuse werden die Controller-Module in der richtigen Reihenfolge installiert. Jedes Controller-Modul versucht, neu zu starten, sobald es vollständig im Gehäuse sitzt.
