...
+
.. Löschen Sie die Daten im Cache-Modul: `system controller flash-cache secure-erase run -node _node_name_ -device-id _device_id_`
.. Stellen Sie sicher, dass die Daten aus dem Caching-Modul gelöscht wurden: `system controller flash-cache secure-erase show -node _node_name_`
+
...
. Wählen Sie im angezeigten Menü die Option **M.2 NVME Drive Stress** aus.
