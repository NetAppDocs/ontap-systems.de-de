...
summary: 'Nachdem Sie den FRU-Austausch in einer Zwei-Knoten MetroCluster -Konfiguration abgeschlossen haben, können Sie die MetroCluster Switchback-Operation durchführen. Dadurch wird die Konfiguration in ihren normalen Betriebszustand zurückversetzt, wobei die virtuellen Maschinen (SVMs) des Sync-Source-Speichers auf dem zuvor beeinträchtigten Standort nun aktiv sind und Daten aus den lokalen Festplattenpools bereitstellen. Das AFF A300 -System unterstützt ausschließlich manuelle Wiederherstellungsverfahren über Bootmedien.' 
---
