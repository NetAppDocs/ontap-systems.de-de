...
---
= Das Chassis in Ihrem AFF A800 System ersetzen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Tauschen Sie das Gehäuse Ihres AFF A800 -Systems aus, wenn ein Hardwaredefekt dies erfordert.  Der Austauschprozess umfasst das Entfernen der Steuergeräte, das Umsetzen der Laufwerke in das Ersatzgehäuse, das Entfernen des defekten Gehäuses, den Einbau des Ersatzgehäuses und den Wiedereinbau der Gehäusekomponenten.



[[step-1-remove-the-controller-modules-from-the-old-chassis]]
== Schritt 1: Entfernen Sie die Controller-Module aus dem alten Gehäuse

Entfernen Sie die Controller-Module aus dem alten Chassis.
...
Bauen Sie die Laufwerke vom alten Gehäuse in das neue Gehäuse ein.
...
Ersetzen Sie das beschädigte Chassis im Geräteschrank oder Systemschrank durch das neue Chassis.
...
== Schritt 4: Installieren Sie die Controller-Module in das neue Gehäuse.

Nachdem die Controller-Module in das neue Chassis eingebaut wurden, wird das Chassis gestartet.

Bei HA-Paaren mit zwei Controller-Modulen im selben Chassis ist die Reihenfolge, in der die Controller-Module installiert werden, besonders wichtig, da jedes Modul versucht, neu zu starten, sobald es vollständig im Chassis sitzt.
...
Nachdem Sie das defekte AFF A800 Gehäuse ausgetauscht und die Komponenten wieder eingebaut haben, müssen Sie Folgendes tun:link:chassis-replace-complete-system-restore-rma.html["Schließen Sie den Austausch des Gehäuses ab"]
