...
[role="lead"]
Übergeben Sie die Kontrolle über die Speicherressourcen wieder an den Ersatzcontroller, damit Ihr AFF A800 -System den normalen Betrieb wiederaufnehmen kann.  Das Rückgabeverfahren variiert je nach Art der von Ihrem System verwendeten Verschlüsselung: keine Verschlüsselung oder Onboard Key Manager (OKM)-Verschlüsselung.
