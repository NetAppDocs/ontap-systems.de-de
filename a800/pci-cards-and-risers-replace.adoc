...
---
= Austausch einer PCIe-Karte in einem AFF A800 System
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Eine PCIe-Karte in Ihrem AFF A800 System wird ersetzt, wenn die Karte defekt ist oder ein Upgrade erforderlich ist. Der Austauschprozess umfasst das Trennen der Kabel und der SFP- oder QSFP-Module, das Ersetzen der Karte und das erneute Verkabeln.

.Bevor Sie beginnen
* Sie benötigen ein neues oder Ersatzteil von NetApp .
* Stellen Sie sicher, dass alle anderen Komponenten des Speichersystems ordnungsgemäß funktionieren. Falls nicht, wenden Sie sich an den technischen Support.
...
Eine PCIe-Karte wird ersetzt, indem die Verkabelung sowie alle QSFPs und SFPs von den Ports der PCIe-Karten im Zielriser entfernt werden, der Riser aus dem Controller-Modul entnommen wird, die PCIe-Karte entfernt und ersetzt wird, der Riser sowie alle QSFPs und SFPs wieder an den Ports installiert und die Ports neu verkabelt werden.
