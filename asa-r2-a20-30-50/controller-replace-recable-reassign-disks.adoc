...
[role="lead"]
Geben Sie die Kontrolle über die Speicherressourcen an den Ersatz-Controller zurück, damit Ihr ASA A20, ASA A30 oder ASA A50-System den normalen Betrieb wieder aufnehmen kann. Das Give-back-Verfahren variiert je nach dem von Ihrem System verwendeten Verschlüsselungstyp: Keine Verschlüsselung, OKM-Verschlüsselung (Onboard Key Manager) oder EKM-Verschlüsselung (External Key Manager).
