...
summary: Übertragen Sie das Boot-Image auf das Ersatz-Bootmedium eines AFF A700s -Systems entweder mit dem sekundären Bootmedium oder mit einem USB-Stick. Das AFF A700s -System unterstützt ausschließlich manuelle Wiederherstellungsverfahren über Bootmedien. Die automatische Wiederherstellung über Bootmedien wird nicht unterstützt. 
---
= Das Boot-Image wird mithilfe eines USB-Sticks auf das Bootmedium in einem AFF A700s System übertragen.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Übertragen Sie das Boot-Image auf das Ersatz-Bootmedium eines AFF A700s-Systems entweder mit dem sekundären Bootmedium (primäre Methode) oder mit einem USB-Stick (falls die Wiederherstellung mit dem sekundären Bootmedium fehlschlägt oder die  `image.tgz` Datei fehlt).
...
== Option 1: Übertragen Sie das Systemabbild mithilfe der Sicherungswiederherstellung vom zweiten Startmedium

Sie können das Systemabbild mithilfe des Abbilds auf dem zweiten Bootmedium im Controllermodul auf das Ersatz-Bootmedium installieren. Dies ist die primäre Methode für Systeme mit zwei Bootmedien im Controllermodul.

Das Image auf dem sekundären Startmedium muss eine  `image.tgz` Datei enthalten und darf keine Fehler melden. Falls die  `image.tgz` Datei fehlt oder das Startmedium Fehler meldet, kann dieses Verfahren nicht angewendet werden. Verwenden Sie <<Option 2: Übertragen Sie das Boot-Image mithilfe eines USB-Sticks>> stattdessen.
...
Dieses Verfahren sollte nur angewendet werden, wenn die Wiederherstellung über das sekundäre Startmedium fehlgeschlagen ist oder wenn die  `image.tgz` Datei auf dem sekundären Startmedium nicht gefunden wird.
...
* Eine Kopie derselben Image-Version von ONTAP, die auf dem betroffenen Controller lief. Sie können das entsprechende Image im Downloadbereich der NetApp Support Site herunterladen.
+
...
.. Speichern Sie die mit dem  `savenv` Befehl geänderten Umgebungsvariablen.
...
. Starten Sie vom Partner-Controller auf der erweiterten Berechtigungsebene die Konfigurationssynchronisierung mit der im vorherigen Schritt aufgezeichneten IP-Adresse: `system node restore-backup -node local -target-address _impaired_node_IP_address_`
. Nachdem die Konfigurationssynchronisation fehlerfrei abgeschlossen ist, drücken Sie `y` Wenn Sie aufgefordert werden, zu bestätigen, dass der Sicherungsvorgang erfolgreich war.
. Überprüfen Sie, ob die Umgebungsvariablen wie erwartet eingestellt sind.
+
