...
summary: Sie müssen ein DIMM im Controller austauschen, wenn in Ihrem Speichersystem Fehler auftreten, wie z. B. übermäßige korrigierbare Fehlerkorrekturcode-Fehler, die auf Health Monitor-Warnungen basieren, oder nicht korrigierbare ECC-Fehler, die typischerweise durch einen einzelnen DIMM-Ausfall verursacht werden und verhindern, dass das Speichersystem ONTAP startet. 
---
= Ein DIMM in Ihrem AFF C190 System ersetzen, um Speicherfehler zu beheben
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
