...
---
= Die NVMEM-Batterie in einem AFF A150 System ersetzen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Die NVMEM-Batterie in Ihrem AFF A150 System sollte ersetzt werden, wenn sie an Ladung verliert oder ausfällt, da sie kritische Systemdaten bei Stromausfällen bewahrt. Der Austauschprozess umfasst das Entfernen des Controller-Moduls, das Ersetzen der Batterie und das erneute Installieren des Controller-Moduls.
...
.. Wenn Sie dies noch nicht getan haben, installieren Sie das Kabelverwaltungsgerät neu.
.. Verbinden Sie die Kabel mit dem Haken- und Schlaufenband mit dem Kabelmanagement-Gerät.
.. Schließen Sie die Stromkabel wieder an die Netzteile und an die Stromquellen an. Schalten Sie die Stromversorgung ein, um den Startvorgang zu starten.
