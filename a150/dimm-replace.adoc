...
summary: Sie müssen ein DIMM im Controller austauschen, wenn Ihr Speichersystem Fehler wie übermäßige CECC-Fehler aufgrund von Health Monitor-Warnungen oder nicht korrigierbare ECC-Fehler feststellt, die typischerweise durch einen einzelnen DIMM-Ausfall verursacht werden, der das Speichersystem am Booten von ONTAP hindert. 
---
= Ein DIMM in Ihrem AFF A150 System ersetzen, um Speicherfehler zu beheben
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ein DIMM in Ihrem AFF A150 System sollte ersetzt werden, wenn das Speichersystem eine übermäßige Anzahl korrigierbarer (CECC) oder nicht korrigierbarer Speicherfehler feststellt. Solche Fehler können verhindern, dass das Speichersystem ONTAP startet.
