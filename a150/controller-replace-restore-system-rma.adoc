...
summary: Um Ihr System wieder voll funktionsfähig zu machen, müssen Sie die NetApp Storage Encryption-Konfiguration wiederherstellen, Lizenzen für den neuen Controller installieren und das defekte Teil an NetApp zurücksenden, wie in den mit dem Kit gelieferten RMA-Anweisungen beschrieben. 
---
= Den Controller-Austausch für ein AFF A150 System abschließen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Um den Controller-Austausch für Ihr AFF A150 System abzuschließen, die NetApp Storage Encryption-Konfiguration (falls erforderlich) wiederherstellen, bestätigen, dass die logischen Schnittstellen (LIFs) an ihren Home-Ports melden, eine Cluster-Integritätsprüfung durchführen und das defekte Teil an NetApp zurücksenden.
