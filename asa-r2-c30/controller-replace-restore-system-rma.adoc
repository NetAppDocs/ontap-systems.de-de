...
summary: Um den Controller-Austausch für Ihr ASA C30-Speichersystem abzuschließen, stellen Sie zunächst die NetApp Storage Encryption-Konfiguration wieder her, falls erforderlich, und installieren Sie die erforderlichen Lizenzen auf dem neuen Controller. Überprüfen Sie anschließend, ob die logischen Schnittstellen an ihren Home-Ports melden, und führen Sie einen Cluster-Integritätscheck durch. Registrieren Sie abschließend die Seriennummer des neuen Controllers und senden Sie das defekte Teil an NetApp zurück. 
---
= Den Controller-Austausch für ein ASA C30 System abschließen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
