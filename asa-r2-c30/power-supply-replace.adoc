...
summary: 'Ersetzen Sie ein AC- oder DC-Netzteil (PSU) in Ihrem ASA C30-Speichersystem, wenn es ausfällt oder defekt wird, und stellen Sie so sicher, dass Ihr System weiterhin die erforderliche Leistung für einen stabilen Betrieb erhält. Beim Austausch wird das fehlerhafte Netzteil von der Stromquelle getrennt, das Netzkabel abgezogen, das fehlerhafte Netzteil ersetzt und dann wieder an die Stromquelle angeschlossen.' 
---
= Ersetzen Sie ein Netzteil - ASA C30
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ersetzen Sie ein AC- oder DC-Netzteil (PSU) in Ihrem ASA C30-Speichersystem, wenn es ausfällt oder defekt wird, und stellen Sie so sicher, dass Ihr System weiterhin die erforderliche Leistung für einen stabilen Betrieb erhält. Beim Austausch wird das fehlerhafte Netzteil von der Stromquelle getrennt, das Netzkabel abgezogen, das fehlerhafte Netzteil ersetzt und dann wieder an die Stromquelle angeschlossen.
