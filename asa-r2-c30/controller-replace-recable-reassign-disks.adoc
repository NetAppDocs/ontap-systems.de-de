...
summary: Übergeben Sie die Kontrolle über die Speicherressourcen an den Ersatzcontroller, damit Ihr ASA C30-Speichersystem den normalen Betrieb wiederaufnehmen kann. Das Rückgabeverfahren hängt vom verwendeten Verschlüsselungstyp Ihres Systems ab, einschließlich keiner Verschlüsselung, Onboard Key Manager-Verschlüsselung oder External Key Manager-Verschlüsselung. 
---
= Den Controller in Ihrem ASA C30 System zurückgeben, damit der normale Betrieb fortgesetzt werden kann.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
