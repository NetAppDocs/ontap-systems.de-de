...
summary: Tauschen Sie die NV-Batterie in Ihrem FAS70- oder FAS90-System aus, wenn der Akku anfängt, aufgeladen zu werden oder ausfällt, da sie für die Aufbewahrung wichtiger Systemdaten bei Stromausfällen verantwortlich ist. Beim Austausch wird der Controller heruntergefahren, das Controller-Modul entfernt, die NV-Batterie ersetzt, das Controller-Modul wieder eingebaut und das fehlerhafte Teil wieder an den NetApp zurückgegeben. 
---
= Die NV-Batterie in einem FAS70 oder FAS90 System ersetzen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
