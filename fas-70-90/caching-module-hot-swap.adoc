...
---
= Den Flash Cache Modulträger oder das Caching-Modul in einem FAS70 oder FAS90 System ersetzen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Bevor Sie beginnen
* Es sollte geprüft werden, ob Ihr Speichersystem über das geeignete Betriebssystem für den Ersatz Flash Cache Modulträger verfügt.
* Es sollte bestätigt werden, dass alle anderen Komponenten ordnungsgemäß funktionieren. Ist dies nicht der Fall, ist der technische Support zu kontaktieren.
...
.Bevor Sie beginnen
* Es ist zu überprüfen, ob das Ersatz-Caching-Modul die gleiche Kapazität wie das ausgefallene Modul hat, entweder vom selben Hersteller oder von einem anderen unterstützten Hersteller.
* Es sollte bestätigt werden, dass alle anderen Komponenten ordnungsgemäß funktionieren. Ist dies nicht der Fall, ist der technische Support zu kontaktieren.
...
.. Das Cache-Modul wird aus dem Gehäuse entfernt, indem der Finger in die Öffnung des Nockenhebels eingehakt und das Modul aus dem Flash Cache Modulträger herausgezogen wird.
+
...
+
Stellen Sie sicher, dass die Befehlsausgabe den Status als  `powered-on` und betriebsbereit meldet.
